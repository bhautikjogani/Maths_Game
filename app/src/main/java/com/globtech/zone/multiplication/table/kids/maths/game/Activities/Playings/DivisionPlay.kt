...
class DivisionPlay : BaseActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AdsManager.show()?.BannerAd(this, binding.adParent, object : BannerAdListener() {
            ...
            override fun onBannerAdLoaded(bannerAdModel: BannerAdModel) {
                ...
                this@DivisionPlay.bannerAdModel = bannerAdModel
                if (!hasWindowFocus()) this@DivisionPlay.bannerAdModel?.onPause()
            }
        })
    ...
    private fun setBindingData() {
        ...
        binding.tvTitle.text = GameType.Division.title
    ...
    private fun doOnGameOver() {
        ...
        val dyKey = GameType.Division.preKey + variationType.preKey
    ...
    private fun getQuestionModel(): QuestionModel {
        ...
        while (true) {
            ...
            postDigit = getQuestion(preDigit.toString().length, preDigit)

            if (isValidDivision(preDigit, postDigit)) break
    ...
    private fun isValidDivision(preDigit: Int, postDigit: Int): Boolean {
        if (postDigit == 0 || preDigit == 0) return false
        return ceil(preDigit.toDouble() / postDigit.toDouble()) == floor(preDigit.toDouble() / postDigit.toDouble())
    }
    ...
    override fun onClick(view: View?) {
        ...
        when (view) {
            ...
            binding.btnHome -> {
                colorAnim?.pause()
                progressAnim?.pause()
                Popup_Conformation(this@DivisionPlay)
            ...
            binding.btnSetting -> {
                colorAnim?.pause()
                progressAnim?.pause()
                Popup_Setting(this@DivisionPlay)
                    .setOnDismissListener {
                        colorAnim?.resume()
                        progressAnim?.resume()
                    }
            ...
            binding.btnDraw -> {
                colorAnim?.pause()
                progressAnim?.pause()
                Popup_DrawView(this@DivisionPlay)
                    .setOnDismissListener {
                        colorAnim?.resume()
                        progressAnim?.resume()
                    }
